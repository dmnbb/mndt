...
func (h *handler) handle() {
	...
	if err != nil {
		fmt.Printf("[ERRO] select dt_event failed err=%v", err)
		return
	}
	defer rows.Close()
	for rows.Next() {
		...
		if err != nil {
			fmt.Printf("[ERRO] select dt_event Scan failed err=%v", err)
			return
		}
		go h.handleOne(de)
	}
...
func (h *handler) handleOne(de *dtEvent) {
	...
	tx, err := h.db.Begin()
	if err != nil {
		fmt.Printf("[ERRO] Begin failed err=%v", err)
		return
	}
	...
	defer func() {
		if err == nil {
			if err = tx.Commit(); err != nil {
				fmt.Printf("[ERRO] Commit failed err=%v", err)
			}
		}
		if err != nil {
			if er := tx.Rollback(); er != nil {
				fmt.Printf("[ERRO] Rollback failed err=%v", er)
			}
		}
	}()

	if err = handleFunc(tx, de.Param); err != nil {
		fmt.Printf("[ERRO] handleFunc failed err=%v", err)
		if er := h.updateDtEventForRetry(de.Id); er != nil {
			fmt.Printf("[ERRO] updateDtEventForRetry failed err=%v", er)
		}
		return
	}

	if err = updateDtEventForState(tx, de.Id); err != nil {
		fmt.Printf("[ERRO] updateDtEventForState failed err=%v", err)
	}
}
